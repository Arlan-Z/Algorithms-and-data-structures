...
public:
    HashMap(int size) {
        capacity = size;
        table.resize(capacity, -1); // Инициализируем все слоты -1 (значение по умолчанию)
    }
    ...
    void insert(int key) { // вставка за константу
        int index = hashFunction(key);
        table[index] = key;
    }

    // Поиск элемента в таблице
    bool search(int key) { //нахождение элемента за константу так как вычисление происходит за постоянное время и нахождение элемента по инексу в векторе за о(1)
        int index = hashFunction(key);
    ...
    void remove(int key) {
        int index = hashFunction(key);
        if (table[index] == key) {
            table[index] = -1; // Удаляем элемент путем установки значения -1
        }
    }
};
```
...
```
**Хэш таблица написанная без библиотеки через пару и свою хэш функцию**
```c++
#include <iostream>
#include <vector>
#include <string>
...
так как hash(s) = (s[0]*x^(n-1)+s[1]*x^(n-2)+s[2]*x^(n-3)+.....+s[n-2]*x+s[n-1])%q
...
void naiveSearch(const string& text, const string& pattern) {
    ...
    for (int i = 0; i <= n - m; ++i) {
        ...
        if (j == m) {
            cout << "Найдено вхождение образца на позиции " << i << endl;
        }
    }
}
```
его временная сложность O(n*m)

Алгоритм Рабина-Карпа — это алгоритм поиска подстроки в тексте на основе хеш-функций. Этот алгоритм использует хеширование для быстрого сравнения подстроки и текста. Его временная сложность O(n + m).
Вот как он работает:
...
здесь мы вычли первый чар то есть s[0]*x^(n-1) после умножили все на x чтобы сдвинуть все в верх и добавили новый элемент s[n]
...
**ОЧЕНЬ ВАЖНО! ДЛЯ НАИМЕНЬШЕГО ПРОЦЕНТА КОЛЛИЗИИ СТАВЬТЕ q ИЛИ ЖЕ MOD БОЛЬШИМ И ПРОСТЫМ ЧИСЛОМ И ОСНОВАНИЕ БОЛЬШЕ АЛФАВИТА**
