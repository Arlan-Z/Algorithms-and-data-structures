...
при хэш функции:  h = (s[0]*x^(n-1)+s[1]*x^(n-2)+s[2]*x^(n-3)+.....+s[n-2]*x+s[n-1])%q
тогда сдвинутый хэш будет h = ((s[1]*x^(n-2)+s[2]*x^(n-3)+.....+s[n-2]*x+s[n-1])*x + s[n])%q здесь мы вычли первый чар то есть s[0]*x^(n-1) после умножили все на x и добавили новый элемент s[n]
