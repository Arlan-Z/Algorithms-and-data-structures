![image](https://github.com/Arlan-Z/Algorithms-and-data-structures/assets/122739941/bd2f61e5-d9b3-4768-9d30-0bab59caa0c8)## DFS
...
![image](https://github.com/Arlan-Z/Algorithms-and-data-structures/assets/122739941/728b9a5a-ef99-4d72-98c2-0875dd2b634a)

Топологическая сортировка - это способ упорядочивания вершин в ориентированном ациклическом графе (DAG) таким образом, что каждое ребро идёт от вершины с меньшим порядковым номером к вершине с большим номером.

Это полезно, например, когда у вас есть зависимости между задачами или событиями: одни задачи должны быть выполнены до начала других. Топологическая сортировка позволяет определить порядок выполнения этих задач.

Здесь представлена реализация топологической сортировки с использованием глубокого поиска в глубину (DFS) для обхода графа. Алгоритм начинает с произвольной вершины и рекурсивно идет вглубь по всем доступным ребрам, помечая вершины как посещенные и добавляя их в стек. После завершения обхода всех вершин алгоритм начинает извлекать вершины из стека, выводя их в порядке, обратном порядку завершения обхода.
...
topologicalSortUtil - это вспомогательная функция, которая используется для рекурсивного обхода графа с целью добавления вершин в стек Stack в порядке, соответствующем топологической сортировке.

v - текущая вершина, которую мы рассматриваем.

visited - хэш-таблица для отслеживания посещенных вершин.

adjList - представление графа в виде списков смежности.

Мы помечаем текущую вершину v как посещенную (visited[v] = true).

Затем мы итерируемся по списку смежности данной вершины (adjList[v]) и для каждой непосещенной смежной вершины вызываем рекурсивно функцию topologicalSortUtil.

После завершения обхода всех смежных вершин текущей вершины, мы помещаем вершину v в стек Stack. Это происходит потому, что в топологической сортировке вершины добавляются в стек в порядке, обратном порядку завершения их обработки.

Функция topologicalSort инициализирует стек Stack и хэш-таблицу visited.

Затем она начинает обход графа. Если вершина не была посещена ранее, вызывается функция topologicalSortUtil для выполнения рекурсивного обхода графа из этой вершины.

После завершения обхода всех вершин функция выводит содержимое стека Stack, который содержит вершины в порядке, соответствующем топологической сортировке.
